...
## Using APIFlask to run as an OpenAPI
Although not strictly an API, since there are not any objects to manage (at least for now), it is still useful
to publish the application endpoints as [OpenAPI](https://www.openapis.org/) endpoints and generate OpenAPI docs and testing interface.
```console
/openapi.json      OpenAPI definition generated from published endpoints
/docs              OpenAPI standard doc and API testing interface
```


It also allows using [py-holder](https://github.com/amesones-dev/py-holder) as an OpenAPI placeholder in backend 
development projects.  


[APIFlask](https://github.com/apiflask/apiflask) is designed to create OpenAPI compatible APIs and uses 
[Flask](https://flask.palletsprojects.com/) as a base, so it is quite convenient when migrating the existing Flask 
application to OpenAPI.
...
* From a browser, use the OpenAPI docs endpoint  
Default URL for local environment:  
  * https://localhost:5000/docs

* If running in [Google Cloud Shell](https://console.cloud.google.com/home/), you can use 
[Cloud Shell Web Preview](https://cloud.google.com/shell/docs/using-web-preview)
  
![OpenAPI interface](docs/res/py-holder-openapi.png)


* Or use *curl* from the shell 
```shell
# HTTP 
